  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      while not coord.should_stop():
        ...
        idx = np.argsort(predictions_val[0])[::-1][:20]
        df = pd.read_csv('./data/vocabulary_youtube_8m.csv')
        for i in idx:
          name.append(df[df['Index']==i]['Name'].values[0])

        pred = pd.DataFrame({'name':np.array(name), 'idx':idx, 'value':predictions_val[0][idx] })
        print('Predict result for video')
        print(pred)
        pred.to_csv('./data/pred/'+str(id)+'.csv')
        break # for smaller set of features files, this is running into race condition, solved by breaking this for each video. 
...
def evaluate():
  ...
  with tf.Graph().as_default():
    ...
    with open(FLAGS.eval_data_pattern) as f:
      dict = json.load(f)
    id = FLAGS.eval_data_pattern.split('/')[-1].split('.')[0]
